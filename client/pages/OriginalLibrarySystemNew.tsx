...
function MemberManagement({ members }: { members: Member[] }) {
  const { deleteMember } = useLibrary();
  const [search, setSearch] = useState("");
  const [deletingMember, setDeletingMember] = useState<string | null>(null);

  const handleDeleteMember = async (id: string, memberName: string) => {
    if (!confirm(`Are you sure you want to delete member "${memberName}"? This will also delete their user account and loan history.`)) {
      return;
    }
    ...
    try {
      ...
      if (!success) {
        alert("Failed to delete member. They may have active borrows.");
      }
    } finally {
    ...
    <Card className="p-6">
      ...
      <div className="rounded-md border">
        <Table>
          ...
          <TableBody>
            {filteredBorrows.filter(borrow => !borrow.returnedAt).map((borrow, index) => (
